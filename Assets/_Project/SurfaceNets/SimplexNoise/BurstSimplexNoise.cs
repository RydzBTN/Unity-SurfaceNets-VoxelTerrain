using Unity.Burst;
using Unity.Collections;
using Unity.Mathematics;

/// <summary>
/// Burst-compatible SimplexNoise
/// </summary>
[BurstCompile]
public struct BurstSimplexNoise
{
    [ReadOnly] private NativeArray<byte> _random;
    [ReadOnly] private NativeArray<int3> _gradients;

    private const int RandomSize = 256;
    private const float F3 = 1.0f / 3.0f;
    private const float G3 = 1.0f / 6.0f;
    
    public BurstSimplexNoise(int seed, Allocator allocator)
    {
        _random = new NativeArray<byte>(RandomSize * 2, allocator);
        _gradients = new NativeArray<int3>(12, allocator);
        _gradients[0] = new int3(1, 1, 0);
        _gradients[1] = new int3(-1, 1, 0);
        _gradients[2] = new int3(1, -1, 0);
        _gradients[3] = new int3(-1, -1, 0);
        _gradients[4] = new int3(1, 0, 1);
        _gradients[5] = new int3(-1, 0, 1);
        _gradients[6] = new int3(1, 0, -1);
        _gradients[7] = new int3(-1, 0, -1);
        _gradients[8] = new int3(0, 1, 1);
        _gradients[9] = new int3(0, -1, 1);
        _gradients[10] = new int3(0, 1, -1);
        _gradients[11] = new int3(0, -1, -1);
        
        int[] source = {
            151, 160, 137, 91, 90, 15, 131, 13,
            201, 95, 96, 53, 194, 233, 7, 225,
            140, 36, 103, 30, 69, 142, 8, 99,
            37, 240, 21, 10, 23, 190, 6, 148,
            247, 120, 234, 75, 0, 26, 197, 62,
            94, 252, 219, 203, 117, 35, 11, 32,
            57, 177, 33, 88, 237, 149, 56, 87,
            174, 20, 125, 136, 171, 168, 68, 175,
            74, 165, 71, 134, 139, 48, 27, 166,
            77, 146, 158, 231, 83, 111, 229, 122,
            60, 211, 133, 230, 220, 105, 92, 41,
            55, 46, 245, 40, 244, 102, 143, 54,
            65, 25, 63, 161, 1, 216, 80, 73,
            209, 76, 132, 187, 208, 89, 18, 169,
            200, 196, 135, 130, 116, 188, 159, 86,
            164, 100, 109, 198, 173, 186, 3, 64,
            52, 217, 226, 250, 124, 123, 5, 202,
            38, 147, 118, 126, 255, 82, 85, 212,
            207, 206, 59, 227, 47, 16, 58, 17,
            182, 189, 28, 42, 223, 183, 170, 213,
            119, 248, 152, 2, 44, 154, 163, 70,
            221, 153, 101, 155, 167, 43, 172, 9,
            129, 22, 39, 253, 19, 98, 108, 110,
            79, 113, 224, 232, 178, 185, 112, 104,
            218, 246, 97, 228, 251, 34, 242, 193,
            238, 210, 144, 12, 191, 179, 162, 241,
            81, 51, 145, 235, 249, 14, 239, 107,
            49, 192, 214, 31, 181, 199, 106, 157,
            184, 84, 204, 176, 115, 121, 50, 45,
            127, 4, 150, 254, 138, 236, 205, 93,
            222, 114, 67, 29, 24, 72, 243, 141,
            128, 195, 78, 66, 215, 61, 156, 180,
        };

        if (seed != 0)
        {
            byte b0 = (byte)(seed & 0xFF);
            byte b1 = (byte)((seed >> 8) & 0xFF);
            byte b2 = (byte)((seed >> 16) & 0xFF);
            byte b3 = (byte)((seed >> 24) & 0xFF);

            for (int i = 0; i < RandomSize; i++)
            {
                _random[i] = (byte)(source[i] ^ b0 ^ b1 ^ b2 ^ b3);
                _random[i + RandomSize] = _random[i];
            }
        }
        else
        {
            for (int i = 0; i < RandomSize; i++)
            {
                _random[i] = (byte)source[i];
                _random[i + RandomSize] = _random[i];
            }
        }
    }
    
    [BurstCompile]
    public float Generate(float3 point)
    {
        float x = point.x;
        float y = point.y;
        float z = point.z;
        float n0 = 0, n1 = 0, n2 = 0, n3 = 0;

        float s = (x + y + z) * F3;

        int i = FastFloor(x + s);
        int j = FastFloor(y + s);
        int k = FastFloor(z + s);

        float t = (i + j + k) * G3;

        float x0 = x - (i - t);
        float y0 = y - (j - t);
        float z0 = z - (k - t);

        int i1, j1, k1;
        int i2, j2, k2;

        if (x0 >= y0)
        {
            if (y0 >= z0)
            {
                i1 = 1; j1 = 0; k1 = 0;
                i2 = 1; j2 = 1; k2 = 0;
            }
            else if (x0 >= z0)
            {
                i1 = 1; j1 = 0; k1 = 0;
                i2 = 1; j2 = 0; k2 = 1;
            }
            else
            {
                i1 = 0; j1 = 0; k1 = 1;
                i2 = 1; j2 = 0; k2 = 1;
            }
        }
        else
        {
            if (y0 < z0)
            {
                i1 = 0; j1 = 0; k1 = 1;
                i2 = 0; j2 = 1; k2 = 1;
            }
            else if (x0 < z0)
            {
                i1 = 0; j1 = 1; k1 = 0;
                i2 = 0; j2 = 1; k2 = 1;
            }
            else
            {
                i1 = 0; j1 = 1; k1 = 0;
                i2 = 1; j2 = 1; k2 = 0;
            }
        }

        float x1 = x0 - i1 + G3;
        float y1 = y0 - j1 + G3;
        float z1 = z0 - k1 + G3;

        float x2 = x0 - i2 + F3;
        float y2 = y0 - j2 + F3;
        float z2 = z0 - k2 + F3;

        float x3 = x0 - 0.5f;
        float y3 = y0 - 0.5f;
        float z3 = z0 - 0.5f;

        int ii = i & 0xFF;
        int jj = j & 0xFF;
        int kk = k & 0xFF;

        float t0 = 0.6f - x0 * x0 - y0 * y0 - z0 * z0;
        if (t0 > 0)
        {
            t0 *= t0;
            int gi0 = _random[ii + _random[jj + _random[kk]]] % 12;
            int3 g0 = _gradients[gi0];
            n0 = t0 * t0 * Dot(g0.x, g0.y, g0.z, x0, y0, z0);
        }

        float t1 = 0.6f - x1 * x1 - y1 * y1 - z1 * z1;
        if (t1 > 0)
        {
            t1 *= t1;
            int gi1 = _random[ii + i1 + _random[jj + j1 + _random[kk + k1]]] % 12;
            int3 g1 = _gradients[gi1];
            n1 = t1 * t1 * Dot(g1.x, g1.y, g1.z, x1, y1, z1);
        }

        float t2 = 0.6f - x2 * x2 - y2 * y2 - z2 * z2;
        if (t2 > 0)
        {
            t2 *= t2;
            int gi2 = _random[ii + i2 + _random[jj + j2 + _random[kk + k2]]] % 12;
            int3 g2 = _gradients[gi2];
            n2 = t2 * t2 * Dot(g2.x, g2.y, g2.z, x2, y2, z2);
        }

        float t3 = 0.6f - x3 * x3 - y3 * y3 - z3 * z3;
        if (t3 > 0)
        {
            t3 *= t3; 
            int gi3 = _random[ii + 1 + _random[jj + 1 + _random[kk + 1]]] % 12;
            int3 g3 = _gradients[gi3];
            n3 = t3 * t3 * Dot(g3.x, g3.y, g3.z, x3, y3, z3);
        }

        return ((n0 + n1 + n2 + n3) * 32.0f);
    }

    
    
    [BurstCompile]
    private static float Dot(int gx, int gy, int gz, float x, float y, float z)
    {
        return gx * x + gy * y + gz * z;
    }

    [BurstCompile]
    private static int FastFloor(float x)
    {
        int xi = (int)x;
        return x < xi ? xi - 1 : xi;
        
        //(int)math.floor(x)
    }

    public void Dispose()
    {
        if (_random.IsCreated) _random.Dispose();
        if (_gradients.IsCreated) _gradients.Dispose();
    }
}
