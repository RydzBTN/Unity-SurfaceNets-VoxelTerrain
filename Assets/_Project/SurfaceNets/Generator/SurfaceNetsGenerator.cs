...
namespace _Project.SurfaceNets.Generator
{
    ...
    {
        public static JobHandle ScheduleChunkMeshGeneration(
            ...
            out NativeReference<Bounds> meshBounds)
        {
            ...
            int totalVoxels = voxelArraySize * voxelArraySize * voxelArraySize;
            int densityCount = densityArraySize * densityArraySize * densityArraySize;

            // temp
            NativeList<float3> vertices = new NativeList<float3>(1000, Allocator.Persistent);
            NativeList<float3> normals = new NativeList<float3>(1000, Allocator.Persistent);
            NativeList<int> triangles = new NativeList<int>(5000, Allocator.Persistent);
            NativeArray<int> voxelIndices = new NativeArray<int>(totalVoxels, Allocator.Persistent);
            ...
            if (!densities.IsCreated)
            {
                ...
                DensityJob densityJob = new DensityJob
                {
                    ...
                    DensityArraySize = densityArraySize,
                    BodyType = (int)type,
                    LodStep = lodStep,
            ...
            MeshJob meshJob = new MeshJob
            {
                ...
                ChunkSize = chunkSize,
                VoxelSize = voxelArraySize,
                DensitySize = densityArraySize,
                LodStep = lodStep,
        ...
        private static (bool isAir, bool isUnder) CheckIsSurface(NativeArray<Point> densities)
        {
            bool hasSolid = false;
            bool hasAir = false;
        ...
        private static void CalculateVertices(NativeArray<Point> densityArray, out Vector3[] vertices,
            out Vector3[] normals, out short[,,] voxelVertexIndices)
        {
        ...
        private static void GenerateTriangles(NativeArray<Point> densityArray, short[,,] voxelVertexIndices,
            out int[] triangles)
        {
